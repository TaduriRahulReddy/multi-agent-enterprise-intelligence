...
# STRICT RAG + PHRASE-AWARE SELF-CORRECTION
# ============================================================
...
def build_research_prompt(
    ...
    1. Direct evidence should normally be treated as sufficient.
    2. Interpretations must remain conservative.
    3. Supporting evidence must contain actual factual evidence.
    """
...
def extract_rejected_phrase(
    ...
    """
    Attempt to extract the exact phrase rejected by a deterministic
    validator message.

    Example:

    Unsupported qualifier detected: 'widely' strengthens the evidence...
...
def build_rewrite_prompt(
    ...
    """
    Build a strict correction prompt after validation failure.

    Version 9 explicitly forbids reuse of a phrase rejected by
...
The grounding validator explicitly rejected this phrase:

"{rejected_phrase}"

You MUST remove this phrase from the rewritten answer.

Do NOT:
- reuse it
- paraphrase it into a stronger equivalent
- preserve the same unsupported meaning using different words

Replace the entire affected sentence with a statement directly supported
...
2. Remove the exact unsupported wording identified by the validator.
...
and make sure the rejected wording and unsupported meaning are gone.
...
def research(
    ...
    5. Extract rejected phrase when possible.
    6. Force a materially different rewrite.
    7. Return grounded answer or safe fallback.
    """

    print("=" * 70)
    print("RESEARCH AGENT - VERSION 9")
    print("STRICT RAG + PHRASE-AWARE SELF-CORRECTION")
    print("=" * 70)
...
You MUST NOT repeat the failed INTERPRETATION.
...
if __name__ == "__main__":
    ...
    print(
        "Testing Research Agent Version 9"
    )
    ...
    print(
        "\nResearch Agent Version 9 testing completed."
    )
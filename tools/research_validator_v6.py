...
# ============================================================
# RESEARCH GROUNDING VALIDATOR - VERSION 6
# CLAIM-LEVEL VALIDATION + META-CONTROL FILTERING
# ============================================================
...
def is_meta_control_statement(
    ...
    These statements describe evidence handling or validation state,
    so they should not be grounded against the business document.
    """
...
def deterministic_grounding_check(
    ...
    """
    Run deterministic checks for obvious unsupported language.
    """
...
def extract_answer_claims(
    ...
    Actual factual statements inside INTERPRETATION are still validated.
    """
    ...
    for raw_line in research_answer.splitlines():
        ...
        # ----------------------------------------------------
        # Ignore evidence sufficiency labels
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Split line into sentence-level claims
        # ----------------------------------------------------
        ...
        for sentence in sentences:
            ...
            # ------------------------------------------------
            # Ignore known meta/control language
            # ------------------------------------------------
...
def extract_json_object(
    ...
    """
    Extract a JSON object from the validator response.
    """
...
def validate_claim_with_llm(
    ...
    """
    Validate exactly one real answer claim against retrieved evidence.
    """
...
1. Judge ONLY the exact text under CLAIM TO VALIDATE.

2. Do not invent another claim.

3. Do not quote or evaluate a statement that is not the supplied claim.

4. Do not require the evidence to contain the exact same wording.
A faithful paraphrase is supported.

5. Do not require the answer to mention every fact in the document.

SUPPORTED means:
- directly stated by the evidence, or
- faithfully paraphrased without changing the meaning.

UNSUPPORTED means the claim introduces:
- a new fact
- a new cause
...
- business impact
- recommendations
- stronger frequency or severity
- a success condition
- operational explanations
- supply-chain explanations
- conclusions not traceable to the evidence
...
Claim:
"Delivery delays reduced customer satisfaction."
...
Claim:
"Management planned to expand pricing if the strategy succeeded."
...
- copy CLAIM TO VALIDATE exactly into the "claim" field
- do not modify it
- evaluate only that claim
...
{{
    ...
    "supported": true,
    "supporting_evidence": "<short matching evidence>",
...
"""
    ...
    # ========================================================
    # EXACT CLAIM SAFETY CHECK
    # ========================================================

    if (
        normalize_text(returned_claim)
...
def validate_research_answer(
    ...
    """
    Validate an entire research answer.
    ...
    # An answer can theoretically contain only meta-control language.
    # That is not automatically a hallucination, but it also contains
    # no factual claim to validate.
    ...
    # ========================================================
    # STEP 3: VALIDATE EACH ACTUAL CLAIM
    # ========================================================
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("RESEARCH GROUNDING VALIDATOR - VERSION 6")
    print("CLAIM-LEVEL + META-CONTROL FILTERING")
    print("=" * 70)

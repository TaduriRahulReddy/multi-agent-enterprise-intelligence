...
def normalize_entity_name(text: str) -> str:
    """
    Normalize entity/category names so:
...
def find_revenue_column(
    ...
    """
    Locate the revenue-like metric column.
    """
...
def result_is_revenue_ranking(
    ...
    """
    Detect a multi-row revenue-ranking style result.
    """
...
def parse_report_sections(
    ...
    """
    Parse report sections and reconstruct wrapped lines.
    """
...
def is_meta_control_sentence(
    ...
    """
    Detect report-control language.
    """
...
def extract_factual_claims(
    ...
    Recommendation sections are excluded.
    """
...
def deterministic_count_check(
    ...
    """
    Prevent count-only data from supporting qualitative
    loyalty / retention / satisfaction claims.
    """
...
def deterministic_revenue_ranking_check(
    ...
    """
    Revenue rankings support numerical values and ordering.

    They do not automatically prove behavioral or market
    interpretations.
    """
...
def deterministic_grounding_check(
    ...
    """
    Run report-level deterministic checks.
    """
...
def find_entity_record_in_claim(
    ...
    """
    Find the entity/category referenced in a factual claim.
    """
    ...
    # Prefer longest category name if multiple names overlap.
    matches.sort(
        key=lambda item: len(
...
def extract_numeric_values(
    ...
    """
    Extract formatted numerical values from a claim.
...
def detect_rank_claim(
    ...
    """
    Detect explicit ranking language.

    Returns expected rank or None.
...
def claim_mentions_revenue(
    ...
    """
    Determine whether claim explicitly discusses revenue.
    """
...
def deterministic_revenue_claim_validation(
    ...
    """
    Validate simple revenue/ranking claims directly in Python.

    This prevents arithmetic/ranking hallucinations from
    the language model.

    Returns:

    handled = True
        Python has enough structure to make the decision.

    handled = False
        Claim should continue to semantic validation.
    """
...
def build_claim_validation_prompt(
    ...
    """
    Build semantic validator prompt for claims Python cannot
    verify structurally.
    """
...
2. Validate ONLY the supplied claim.
...
def parse_json_validation_response(
    ...
    """
    Parse LLM claim validation output.
    """
...
def validate_single_claim(
    ...
    1. Attempt deterministic numeric/rank validation.
    2. Only use the language model when Python cannot
       safely determine the claim.
    """
...
def recommendation_has_unsupported_premise(
    ...
    """
    Recommendations may suggest further analysis, but
    cannot rely on unsupported factual premises.
    """
...
def validate_report(
    ...
    For Each Claim:
        ↓
    Can Python Verify Numeric/Rank Claim?
        ├── Yes → Deterministic Validation
        └── No  → Claim-Level LLM Validation
        ↓
    Recommendation Validation
        ↓
    VALID / INVALID
    ...
    return {
        ...
        "source": "hybrid_claim_validator_v6",
        "claim_results": claim_results,
        "recommendation_results": (
            recommendation_validation[
                "recommendation_results"
...
# ============================================================
# LOCAL TESTS - REPORT VALIDATOR VERSION 6
# ============================================================
...
if __name__ == "__main__":
    ...
    print(
        "Testing Report Validator Version 6"
    )
